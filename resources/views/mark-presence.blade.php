    ...
    <div class="py-12">
        <div
            ...
            <form class="mt-8 p-5 w-full"
                ...
                @endif
                @csrf
                <input id="user" value="{{$user}}" name="user" type="hidden" required>
                ...
                <div class="box flex">
                    ...
                    <div class="w-2/3 pl-5 flex">
                        ...
                        <div class="w-1/2">
                            <div>{{Request::is('enrg3') ? $user->3a_nom : $user->3a_nom}}</div>
                            <div class="pt-3">{{Request::is('enrg3') ? $user->3a_cin : $user->4a_cin}}</div>
                            <div class="pt-3">{{Request::is('enrg3') ? $user->3a_cne : $user->4a_cne}}</div>
                            <div class="pt-3">{{Request::is('enrg3') ? $user->3a_cin : $user->4a_cne}}</div>
                            <div class="pt-3">{{Request::is('enrg3') ? $user->3a_cin : $user->4a_cne}}</div>
                        </div>
